...
interface CalendarContextType {
  ...
  updateFilters: (
    filterType: "platforms" | "clients" | "labels",
    values: string[]
  ) => void;
}
...
export const CalendarProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [platforms, setPlatforms] =
    useState<SocialMediaPlatform[]>(initialPlatforms);
  ...
  const updateEvent = (updatedEvent: CalendarEvent) => {
    ...
    if (updatedEvent.repeat || updatedEvent.parentEventId) {
      ...
      const filteredEvents = events.filter(
        (e) => e.id !== parentId && e.parentEventId !== parentId
      );
      ...
      if (eventToDelete) {
        if (eventToDelete.parentEventId) {
          ...
          setEvents((prev) =>
            prev.filter(
              (e) =>
                e.id !== eventToDelete.parentEventId &&
                e.parentEventId !== eventToDelete.parentEventId
            )
          );
        } else {
          // This is a parent event, delete all children
          setEvents((prev) =>
            prev.filter((e) => e.id !== id && e.parentEventId !== id)
          );
        }
      }
    } else {
  ...
  const filteredEvents = events.filter((event) => {
    ...
    const matchesPlatform =
      filters.platforms.length === 0 ||
      event.platforms.some((platformId) =>
        filters.platforms.includes(platformId)
      );

    // Check if event matches client filter
    const matchesClient =
      filters.clients.length === 0 || filters.clients.includes(event.clientId);
